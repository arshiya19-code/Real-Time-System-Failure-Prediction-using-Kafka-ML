...
Dataset Construction Pipeline v4.0 - Ultra Memory Efficient
Algorithm: Conditional Gaussian Augmentation (CGA)
Base Data: HDFS_v1 (Loghub, Zhu et al., ISSRE 2023)
"""
...
def parse_logs(lmap):
    print(f"\n[STEP 2] Parsing logs (max {MAX_ROWS:,} rows, chunks of {CHUNK_SIZE:,})...")
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    tmp = os.path.join(OUTPUT_DIR,"temp.csv")
    ...
    os.remove(tmp)
    print(f"         Normal: {(df.anomaly_label==0).sum():,} | Anomaly: {(df.anomaly_label==1).sum():,}")
    return df
...
def export(df):
    ...
    df.to_csv(os.path.join(OUTPUT_DIR,"structured_logs.csv"),index=False)
    tr.to_csv(os.path.join(OUTPUT_DIR,"train_logs.csv"),index=False)
    te.to_csv(os.path.join(OUTPUT_DIR,"test_logs.csv"),index=False)
    print(f"[STEP 5] Exported: {len(df):,} rows | {len(cols)} cols")
    print(f"         Normal: {(df.anomaly_label==0).sum():,} | Anomaly: {(df.anomaly_label==1).sum():,} ({df.anomaly_label.mean()*100:.1f}%)")
    print(f"         Train: {len(tr):,} | Test: {len(te):,}")
    print(f"         Saved to: {OUTPUT_DIR}")

def main():
    print("="*55)
    print(" DATASET BUILDER v4.0 — Ultra Memory Efficient")
    print(" Algorithm: Conditional Gaussian Augmentation (CGA)")
    print(" Base: HDFS_v1 (Loghub, ISSRE 2023)")
    print("="*55+"\n")
    lmap = load_labels()
    df   = parse_logs(lmap)
    df   = augment(df)
