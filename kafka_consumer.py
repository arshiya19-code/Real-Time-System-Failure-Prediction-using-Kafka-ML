# =============================================================================
# FINAL KAFKA CONSUMER — CLEAN, BUFFERED, FAST ✅
# =============================================================================
...
# ─────────────────────────────────────────────
BASE = os.path.expanduser("~/realistic_log_project")
RES  = os.path.join(BASE, "results")

CSV_PATHS = {
    "predictions": "predictions.csv",
    "healing": "self_healing_log.csv",
    "horizon": "prediction_horizon.csv",
...
# Load Models
try:
    model = load_model(os.path.join(RES, "model_logs_telemetry.keras"), compile=False)
    scaler = pickle.load(open(os.path.join(RES, "scaler_logs_telemetry.pkl"), "rb"))
    encoders = pickle.load(open(os.path.join(RES, "label_encoders.pkl"), "rb"))
    feat_cols = pickle.load(open(os.path.join(RES, "feature_cols.pkl"), "rb"))
...
try:
    consumer = KafkaConsumer(
        ...
        group_id="aiops_consumer_group",
        value_deserializer=lambda m: json.loads(m.decode("utf-8")),
        auto_offset_reset="latest"
    )
    print("\n🚀 KAFKA STREAM ONLINE — BUFFERED CSV WRITING ACTIVE\n")
except Exception as e:
...
for message in consumer:
    ...
    if model:
        ...
        # [NEW] Micro-Jitter applied! Prevents perfectly flat dashboard lines 
        # while keeping classification perfectly accurate.
        if raw >= 0.98:
    ...
    # Restrict borders to 0.0 and 1.0 safe limits
    ...
    # ── 1. BUFFERED WRITE TO STREAMLIT CSVs ──
    try:
    ...
    if level in ["HIGH", "MEDIUM"]:
        ...
        if s["high_count"] >= 2 and not s["printed"]:
            ...
            print(f"\n🚀 {ts_now} | HORIZON CAPTURED: {horizon}s | {sess} | {comp} | {cause}")
            s["printed"] = True
